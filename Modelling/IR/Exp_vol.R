...
CITES_Vol <- data.table::fread("Data/All_Vertebrates/Cleaned/IR/CITES_Vert_Exporter_dat_IR.csv", na.strings = "")
...
## But actually this doesnt matter, Importer reported exports should be from 0 - 20 any way as a current party could be reported 
## by a party so it could be exported when it was a non party then when it was a party
Check <- left_join(CITES_Vol, NLP_dat, by = c("Exporter" = "ISO", "Year" = "Year")) %>%
  filter(is.na(Category)) %>% group_by(Exporter, Year) %>% tally()
...
backbone <- expand.grid(unique(Country_dat_Aves$Exporter), unique(Country_dat_Aves$Year)) %>%
  rename(Exporter = 1, Year = 2) %>% left_join(NLP_dat, by = c("Exporter" = "ISO", "Year" = "Year")) %>%
...
length(unique(Country_dat_full_vol_aves$Exporter)) ## 130
Country_dat_full_vol_aves %>% filter(is.na(Country))
...
## Series length - Eritrea has been a cites party our whole series but only traded one species that had
## a shorter series, so we do want to expand that eritrea series to the full length.
backbone <- expand.grid(unique(Country_dat_Amph$Exporter), unique(Country_dat_Amph$Year)) %>%
  rename(Exporter = 1, Year = 2) %>% left_join(NLP_dat, by = c("Exporter" = "ISO", "Year" = "Year")) %>%
  mutate(Category = ifelse(is.na(Category), "NP", Category))
...
Country_dat_full_vol_amph %>% filter(is.na(Country))
...
## Series length - Eritrea has been a cites party our whole series but only traded one species that had
## a shorter series, so we do want to expand that eritrea series to the full length.
backbone <- expand.grid(unique(Country_dat_Mam$Exporter), unique(Country_dat_Mam$Year)) %>%
  rename(Exporter = 1, Year = 2) %>% left_join(NLP_dat, by = c("Exporter" = "ISO", "Year" = "Year")) %>%
  mutate(Category = ifelse(is.na(Category), "NP", Category))
...
## Series length - Eritrea has been a cites party our whole series but only traded one species that had
## a shorter series, so we do want to expand that eritrea series to the full length.
backbone <- expand.grid(unique(Country_dat_Rept$Exporter), unique(Country_dat_Rept$Year)) %>%
  rename(Exporter = 1, Year = 2) %>% left_join(NLP_dat, by = c("Exporter" = "ISO", "Year" = "Year")) %>%
  mutate(Category = ifelse(is.na(Category), "NP", Category))
...
Mod_Exp_vol_Aves <- brm(bf(vol ~ 1 + SYear*Category1 + (1|Year) + (1 + SYear|Country),
...
write.csv(Country_dat_full_vol_amph, "Models/IR/Fitting_data/Exp_Vol_Amph.csv", na = "")
write.csv(Country_dat_full_vol_mam, "Models/IR/Fitting_data/Exp_Vol_Mam.csv", na = "")
write.csv(Country_dat_full_vol_rept, "Models/IR/Fitting_data/Exp_Vol_Rept.csv", na = "")

#### Checking ####

pp_check(Mod_Exp, type = "stat")

pp_check(Mod_Exp)+ coord_cartesian(xlim=c(0,100000))

prop_zero <- function(x) {sum(x == 0)/length(x)}
ppc_stat(y = Country_dat_full$vol, yrep = posterior_predict(Mod_Exp, draws = 500), stat="prop_zero")

dispersion<- function(x) {var(x)/mean(x)}
ppc_stat(y = Country_dat_full$vol, yrep = posterior_predict(Mod_Exp, draws = 500), stat="dispersion")


C_Fit <- add_epred_draws(Mod_Exp_vol_Rept, newdata =  Country_dat_full_vol_rept, re_formula = NULL, ndraws = 200)  %>% 
...
ggplot(C_Fit, aes(SYear, .epred)) +
  ...
  geom_point(data = Country_dat_full_vol_rept, aes(SYear, vol)) +
