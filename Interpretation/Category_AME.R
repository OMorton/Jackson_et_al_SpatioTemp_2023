...
marginal_effect_delta <- function(fitting_data, model, eps = 0.0001) {
  ...
  return(list("Cat_ME_Sum_Raw" = Cat_ME_Sum_Raw, "Cat_ME_Sum" = Cat_ME_Sum, "Cat_ME_Sum_3yr" = Cat_ME_Sum_3yr,
              "Cat_ME_Y" = Cat_ME_Y, "Cat_ME_N" = Cat_ME_N))
}
...
Category_plt <- ggarrange(empty, empty, Aves_Exp_plots$Inset_plot, Aves_Imp_plots$Inset_plot, 
                          ...
                          empty, empty, Amph_Exp_plots$Inset_plot, Amph_Imp_plots$Inset_plot, 
...
ggsave(path = "Outputs/Figure_4", Category_plt3, filename = "Figure_4.png",  bg = "white",
       device = "png", width = 30, height = 30, units = "cm")

#### Write out ####
write.csv(Aves_Exp_ME$Cat_ME_Sum, "Outputs/Figure_4/Aves_Exp_deltaME.csv", na = "")
write.csv(Mam_Exp_ME$Cat_ME_Sum, "Outputs/Figure_4/Mam_Exp_deltaME.csv", na = "")
write.csv(Amph_Exp_ME$Cat_ME_Sum, "Outputs/Figure_4/Amph_Exp_deltaME.csv", na = "")
write.csv(Rept_Exp_ME$Cat_ME_Sum, "Outputs/Figure_4/Rept_Exp_deltaME.csv", na = "")

write.csv(Aves_Imp_ME$Cat_ME_Sum, "Outputs/Figure_4/Aves_Imp_deltaME.csv", na = "")
write.csv(Mam_Imp_ME$Cat_ME_Sum, "Outputs/Figure_4/Mam_Imp_deltaME.csv", na = "")
write.csv(Amph_Imp_ME$Cat_ME_Sum, "Outputs/Figure_4/Amph_Imp_deltaME.csv", na = "")
write.csv(Rept_Imp_ME$Cat_ME_Sum, "Outputs/Figure_4/Rept_Imp_deltaME.csv", na = "")

## Combined files
rbind(Aves_Exp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Aves"),
Mam_Exp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Mammalia"),
Amph_Exp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Amphibia"),
Rept_Exp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Reptilia")) %>%
  select(Class, Year, delta_ME, .lower, .upper, PD) %>% 
  write.csv("Outputs/Figure_4/deltaME_Exp_Sum.csv", na = "")

rbind(Aves_Imp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Aves"),
      Mam_Imp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Mammalia"),
      Amph_Imp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Amphibia"),
      Rept_Imp_ME$Cat_ME_Sum %>% filter(Year %in% c(0, 10, 20)) %>% mutate(Class = "Reptilia")) %>%
  select(Class, Year, delta_ME, .lower, .upper, PD) %>% 
  write.csv("Outputs/Figure_4/deltaME_Imp_Sum.csv", na = "")

#### Supplementary tables ####

## Function for summarizing the ME of countries with and without legislation
ME_Sum_YN <- function(ME_data = data, fitting_data = data2){
  Out <- posteriordraws(ME_data) %>% 
  mutate(G_ME = draw/sd(as.numeric(as.character(fitting_data$Year)))) %>%
  group_by(Year) %>%
  mutate(PD = round((sum(sign(G_ME) == sign(median(G_ME)))/n())*100, digits = 2)) %>%
  group_by(Year, PD) %>% 
...
## Exp
Aves_Y_Exp <- ME_Sum_YN(ME_data = Aves_Exp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_aves) %>% 
  mutate(Class = "Aves", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Aves_N_Exp <- ME_Sum_YN(ME_data = Aves_Exp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_aves) %>% 
  mutate(Class = "Aves", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Mam_Y_Exp <- ME_Sum_YN(ME_data = Mam_Exp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_mam) %>% 
  mutate(Class = "Mammalia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Mam_N_Exp <- ME_Sum_YN(ME_data = Mam_Exp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_mam) %>% 
  mutate(Class = "Mammalia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Amph_Y_Exp <- ME_Sum_YN(ME_data = Amph_Exp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_amph) %>% 
  mutate(Class = "Amphibia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Amph_N_Exp <- ME_Sum_YN(ME_data = Amph_Exp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_amph) %>% 
  mutate(Class = "Amphibia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Rept_Y_Exp <- ME_Sum_YN(ME_data = Rept_Exp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_rept) %>% 
  mutate(Class = "Reptilia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Rept_N_Exp <- ME_Sum_YN(ME_data = Rept_Exp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_rept) %>% 
  mutate(Class = "Reptilia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

## Imp
Aves_Y_Imp <- ME_Sum_YN(ME_data = Aves_Imp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_aves_IMP) %>% 
  mutate(Class = "Aves", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Aves_N_Imp <- ME_Sum_YN(ME_data = Aves_Imp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_aves_IMP) %>% 
  mutate(Class = "Aves", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Mam_Y_Imp <- ME_Sum_YN(ME_data = Mam_Imp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_mam_IMP) %>% 
  mutate(Class = "Mammalia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Mam_N_Imp <- ME_Sum_YN(ME_data = Mam_Imp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_mam_IMP) %>% 
  mutate(Class = "Mammalia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Amph_Y_Imp <- ME_Sum_YN(ME_data = Amph_Imp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_amph_IMP) %>% 
  mutate(Class = "Amphibia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Amph_N_Imp <- ME_Sum_YN(ME_data = Amph_Imp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_amph_IMP) %>% 
  mutate(Class = "Amphibia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

Rept_Y_Imp <- ME_Sum_YN(ME_data = Rept_Imp_ME$Cat_ME_Y, fitting_data = Country_dat_full_vol_rept_IMP) %>% 
  mutate(Class = "Reptilia", Category1 = "Yes") %>% filter(Year %in% c(0, 10, 20))
Rept_N_Imp <- ME_Sum_YN(ME_data = Rept_Imp_ME$Cat_ME_N, fitting_data = Country_dat_full_vol_rept_IMP) %>% 
  mutate(Class = "Reptilia", Category1 = "No") %>% filter(Year %in% c(0, 10, 20))

## Write out
write.csv(select(rbind(Aves_Y_Exp, Aves_N_Exp, Mam_Y_Exp, Mam_N_Exp, Amph_Y_Exp, Amph_N_Exp, Rept_Y_Exp, Rept_N_Exp),
                 c(Class, Category1, Year, G_ME, .upper, .lower, PD)), 
          "Outputs/Figure_4/Exp_CategoryME.csv", na = "")

write.csv(select(rbind(Aves_Y_Imp, Aves_N_Imp, Mam_Y_Imp, Mam_N_Imp, Amph_Y_Imp, Amph_N_Imp, Rept_Y_Imp, Rept_N_Imp),
                 c(Class, Category1, Year, G_ME, .upper, .lower, PD)), 
          "Outputs/Figure_4/Imp_CategoryME.csv", na = "")
