...
CITES_Exp_dat <- data.table::fread("Data/All_Vertebrates/Cleaned/IR/CITES_Vert_Exporter_dat_IR.csv", na.strings = "") %>% 
  rename(Country = name, iso = Exporter)

CITES_Imp_dat <- data.table::fread("Data/All_Vertebrates/Cleaned/IR/CITES_Vert_Importer_dat_IR.csv", na.strings = "") %>% 
...
plot_map <- function(data, metric = "SR", type = "Exporter") {
  ...
  Map <- ggplot() + geom_sf(data = data$Country_vol_totals, aes(fill = vol), colour = NA) +
    scale_fill_viridis(name = "Volume", trans = "log10", na.value="grey90",
                       option = "viridis", 
                       breaks = c(1, 100, 100000), labels = c(1, 100, 100000), limits = c(1, 14000000)) +
  ...
  if (metric == "SR" & type == "Exporter") {
    Map <- ggplot() + geom_sf(data = data$Country_vol_totals, aes(fill = Richness), colour = NA) +
      scale_fill_viridis(name = "Volume", trans = "log10", na.value="grey90",
                         option = "viridis", 
                         breaks = c(1, 10, 100), labels = c(1, 10, 100), limits = c(1, 130)) +
      coord_sf(ylim = c(-50, 90), datum = NA) +
      theme_classic(base_size = 12) +
      theme(panel.grid = element_blank(), legend.position = "none")
  }
...
Exp_min_max <- rbind(as.data.frame(CITES_Aves_Exp_Sum$Country_vol_totals), as.data.frame(CITES_Mam_Exp_Sum$Country_vol_totals), 
                     as.data.frame(CITES_Amph_Exp_Sum$Country_vol_totals), as.data.frame(CITES_Rept_Exp_Sum$Country_vol_totals)) %>%
  ungroup() %>% filter(!is.na(Richness), !is.na(vol)) %>%
  summarise(min_SR = min(Richness), max_SR = max(Richness),
            min_vol = min(vol), max_vol = max(vol))

Imp_min_max <- rbind(as.data.frame(CITES_Aves_Imp_Sum$Country_vol_totals), as.data.frame(CITES_Mam_Imp_Sum$Country_vol_totals), 
                     as.data.frame(CITES_Amph_Imp_Sum$Country_vol_totals), as.data.frame(CITES_Rept_Imp_Sum$Country_vol_totals)) %>%
  ungroup() %>% filter(!is.na(Richness), !is.na(vol)) %>%
  summarise(min_SR = min(Richness), max_SR = max(Richness),
            min_vol = min(vol), max_vol = max(vol))           
...
Legend_vol <- get_legend(ggplot() + geom_sf(data = CITES_Aves_Exp_Sum$Country_vol_totals, aes(fill = vol), colour = NA) +
  scale_fill_viridis(name = "Volume", trans = "log10", na.value="grey90",
                     option = "viridis", 
                     breaks = c(1, 100, 10000, 1000000), labels = c(1, 100, 10000, 1000000), limits = c(1, 14000000)) +
  coord_sf(ylim = c(-50, 90), datum = NA) +
...
Map_Vol_arrange2 <- Map_Vol_arrange + annotation_custom(Bird, xmin = 0.95, xmax = 0.99, ymin = 0.9, ymax = 1) +
  ...
  annotation_custom(text_grob("n = 130", face = "italic", size = 11), xmin = 0.25, xmax = 0.25, ymin = 0.77, ymax = 0.77) +
  annotation_custom(text_grob("n = 105", face = "italic", size = 11), xmin = 0.25, xmax = 0.25, ymin = 0.52, ymax = 0.52) +
  annotation_custom(text_grob("n = 9", face = "italic", size = 11), xmin = 0.25, xmax = 0.25, ymin = 0.27, ymax = 0.27) +
  annotation_custom(text_grob("n = 116", face = "italic", size = 11), xmin = 0.25, xmax = 0.25, ymin = 0.02, ymax = 0.02) +
  annotation_custom(text_grob("n = 98", face = "italic", size = 11), xmin = 0.75, xmax = 0.75, ymin = 0.77, ymax = 0.77) +
  annotation_custom(text_grob("n = 105", face = "italic", size = 11), xmin = 0.75, xmax = 0.75, ymin = 0.52, ymax = 0.52) +
  annotation_custom(text_grob("n = 19", face = "italic", size = 11), xmin = 0.75, xmax = 0.75, ymin = 0.27, ymax = 0.27) +
  annotation_custom(text_grob("n = 86", face = "italic", size = 11), xmin = 0.75, xmax = 0.75, ymin = 0.02, ymax = 0.02)
...
ggsave(path = "Outputs/SOM/IR", Map_Vol_arrange3, filename = "Figure1_IR.png",  bg = "white",
       device = "png", width = 30, height = 30, units = "cm")

ggsave(path = "Outputs/SOM/IR", Map_SR_arrange3, filename = "Figure1_SR_IR.png",  bg = "white",
       device = "png", width = 30, height = 30, units = "cm")
...
write.csv(all_correlations, "Outputs/SOM/IR/SR_Vol_Correlations.csv", na = "")
...
ggsave(path = "Outputs/SOM/IR", XY_arrangement, filename = "Figure1_SR_Vol_XY_IR.png",  bg = "white",
       device = "png", width = 30, height = 30, units = "cm")
