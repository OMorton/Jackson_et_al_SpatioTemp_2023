...
#### Read in ####
G_ME_Sum_Aves_EXP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Aves_EXP.csv") %>%
  rename("iso" = "Exporter") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
G_ME_Sum_Amph_EXP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Amph_EXP.csv") %>%
  rename("iso" = "Exporter") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
G_ME_Sum_Mam_EXP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Mam_EXP.csv") %>%
  rename("iso" = "Exporter") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
G_ME_Sum_Rept_EXP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Rept_EXP.csv") %>%
  rename("iso" = "Exporter") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))

G_ME_Sum_Aves_IMP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Aves_IMP.csv") %>%
  rename("iso" = "Importer") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
G_ME_Sum_Amph_IMP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Amph_IMP.csv") %>%
  rename("iso" = "Importer") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
G_ME_Sum_Mam_IMP <- data.table::fread("Outputs/G-ME/IR/G_ME_Sum_Mam_IMP.csv") %>%
  rename("iso" = "Importer") %>% mutate(iso = ifelse(is.na(iso), "NA", iso))
...
Aves_Exp_top <- ggplot(filter(G_ME_Sum_Aves_EXP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  annotate("text", label = "Senegal", x = -30000, y = 1000000, colour = "royalblue3") +
  annotate("text", label = "Guinea", x = -60000, y = 825214, colour = "royalblue3") +
  annotate("text", label = "Mali", x = -15000, y = 500080, colour = "royalblue3") +
...
Amph_Exp_top <- ggplot(filter(G_ME_Sum_Amph_EXP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  annotate("text", label = "Madagascar", x = 500, y =  190348, colour = "royalblue3") +
  annotate("text", label = "Suriname", x = 500, y = 21288, colour = "steelblue2") +
  ylab("Total volume") +
  xlab("Country marginal effect of year (2000 - 2020)") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor = element_blank())
...
Rept_exp_bar <- ggplot(Rept_bar_sum, aes(x = n, y = Col, fill = Interpretation)) + 
  ...
  annotate("text", label = "21", x = 21, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "39", x = 38, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "40", x = 41, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "52", x = 52, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  coord_cartesian(ylim = c(0.5, 2.2)) +
  theme_void() +
  theme(legend.position = "none")
...
Aves_Imp_top <- ggplot(filter(G_ME_Sum_Aves_IMP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  scale_colour_manual(values = c("tomato", "darkgoldenrod", "grey40",  "steelblue2", "royalblue3"), drop=FALSE) +
  scale_y_log10(breaks = c(1000000, 100000, 10000), limits = c(10000, 1000000)) +
  coord_cartesian(xlim = c(-220000, 65000), expand = TRUE) + 
  annotate("text", label = "Spain", x = -200000, y =  661531, colour = "royalblue3") +
  annotate("text", label = "Mexico", x = 45000, y = 650866, colour = "grey40") +
  annotate("text", label = "Portugal", x = -100000, y =  477180, colour = "royalblue3") +
  ylab("Total volume") +
  xlab("Country marginal effect of year (2000 - 2020)") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor = element_blank())
...
Aves_imp_bar <- ggplot(Aves_bar_sum_imp, aes(x = n, y = Col, fill = Interpretation)) + 
  ...
  annotate("text", label = "0", x = 0, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "18", x = 18, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "25", x = 24, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  annotate("text", label = "26", x = 26, y = 1.9, colour = "black", fontface = 'bold', size = 4) +
  coord_cartesian(ylim = c(0.5, 2.2)) +
  theme_void() +
  theme(legend.position = "none")
...
Mam_Imp_top <- ggplot(filter(G_ME_Sum_Mam_IMP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  annotate("text", label = "Italy", x = -27000, y =  1098952, colour = "royalblue3") +
  annotate("text", label = "Turkey", x = 6000, y = 923914, colour = "grey40") +
  annotate("text", label = "Germany", x = -16000, y =  788952, colour = "royalblue") +
  annotate("text", label = "Hong Kong", x = -17000, y = 447528, colour = "royalblue3") +
  ylab("Total volume") +
  xlab("Country marginal effect of year (2000 - 2020)") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor = element_blank())
...
Amph_Imp_top <- ggplot(filter(G_ME_Sum_Amph_IMP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  geom_point(shape = 16, size = 2) + 
  scale_colour_manual(values = c("tomato", "darkgoldenrod", "grey40",  "steelblue2", "royalblue3"), drop=FALSE) +
  scale_y_log10(breaks = c(1000000, 100000, 10000), limits = c(10000, 300000)) +
  coord_cartesian(xlim = c(-2300, 2000/2.5), expand = TRUE) + 
  annotate("text", label = "USA", x = -1900, y =  168451, colour = "royalblue3") +
  annotate("text", label = "Japan", x = 300, y = 15213, colour = "grey40") +
  annotate("text", label = "Canada", x = -500, y =  14464, colour = "steelblue2") +
  ylab("Total volume") +
  xlab("Country marginal effect of year (2000 - 2020)") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor = element_blank())
...
Rept_Imp_top <- ggplot(filter(G_ME_Sum_Rept_IMP, n >= 10000), aes(G_ME, n, colour = Interpretation)) + 
  ...
  scale_colour_manual(values = c("tomato", "darkgoldenrod", "grey40",  "steelblue2", "royalblue3"), drop=FALSE) +
  scale_y_log10(breaks = c(1000000, 100000, 10000)) +
  coord_cartesian(xlim = c(-120000, 125000), expand = TRUE) + 
  annotate("text", label = "Singapore", x = -100000, y =  12682462, colour = "grey40") +
  annotate("text", label = "USA", x = -70000, y = 5564522, colour = "steelblue2") +
  annotate("text", label = "Italy, Mexico, France", x = 60000, y =  5068225, colour = "grey40") +
  annotate("text", label = "Hong Kong, China", x = 90000, y =  3531128, colour = "tomato") +
  ylab("Total volume") +
  xlab("Country marginal effect of year (2000 - 2020)") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.minor = element_blank())
...
ggsave(path = "Outputs/SOM/IR", XY_Arrange3, filename = "Figure3_altColRange_IR.png",  bg = "white",
